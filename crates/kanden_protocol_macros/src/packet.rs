...
pub(super) fn derive_packet(item: TokenStream) -> Result<TokenStream> {
    ...
    let state = packet_attr
        .state
        .unwrap_or_else(|| parse_quote!(::kanden_protocol::PacketState::Play));
    ...
    let packet_id: Expr = match packet_attr.id {
        ...
        None => match syn::parse_str::<Ident>(&(suffix + &*name_str).to_shouty_snake_case()) {
            Ok(ident) => parse_quote!(::kanden_protocol::packet_id::#ident),
            Err(_) => {
    ...
    let side = if let Some(side_attr) = packet_attr.side {
        ...
        parse_quote!(::kanden_protocol::PacketSide::Clientbound)
    } else if name_str.to_lowercase().ends_with("c2s") {
        parse_quote!(::kanden_protocol::PacketSide::Serverbound)
    } else {
        return Err(Error::new(
            packet_attr.span,
    ...
    Ok(quote! {
        impl #impl_generics ::kanden_protocol::__private::Packet for #name #ty_generics
        #where_clause
        {
            const ID: i32 = #packet_id;
            const NAME: &'static str = #name_str;
            const SIDE: ::kanden_protocol::PacketSide = #side;
            const STATE: ::kanden_protocol::PacketState = #state;
        }
    })
}
