...
pub(super) fn derive_decode(item: TokenStream) -> Result<TokenStream> {
    ...
    match input.data {
        Data::Struct(struct_) => {
            ...
            add_trait_bounds(
                &mut input.generics,
                quote!(::kanden_protocol::__private::Decode<#lifetime>),
            );
            ...
            Ok(quote! {
                ...
                impl #impl_generics ::kanden_protocol::__private::Decode<#lifetime> for #input_name #ty_generics
                #where_clause
                {
                    fn decode(_r: &mut &#lifetime [u8]) -> ::kanden_protocol::__private::Result<Self> {
                        use ::kanden_protocol::__private::{Decode, Context, ensure};
        ...
        Data::Enum(enum_) => {
            ...
            Ok(quote! {
                ...
                {
                    fn decode(_r: &mut &#lifetime [u8]) -> ::kanden_protocol::__private::Result<Self> {
                        use ::kanden_protocol::__private::{Decode, Context, VarInt, bail};
