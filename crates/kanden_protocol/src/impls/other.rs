...
use kanden_generated::block::{BlockEntityKind, BlockKind, BlockState};
use kanden_generated::item::ItemKind;
use kanden_generated::registry_id::RegistryId;
use kanden_ident::{Ident, IdentError};
use kanden_nbt::Compound;
use kanden_text::color::RgbColor;
...
impl Encode for Compound {
    fn encode(&self, w: impl Write) -> anyhow::Result<()> {
        Ok(kanden_nbt::to_binary(self, w, None::<&'static str>)?)
    }
}
