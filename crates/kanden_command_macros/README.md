Simplify the creation of Kanden commands with a derive macro.
...
For example, in the `Teleport` enum, the variants are assigned the scope `kanden:command:teleport`, which means they
can be used by anyone with the `kanden:command:teleport`, `kanden:command` or `kanden` scope.
...
leaf is the command. See the documentation for `command.rs` in `kanden_command` for more information.
