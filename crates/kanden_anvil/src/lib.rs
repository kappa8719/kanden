...
use lru::LruCache;
use thiserror::Error;
use kanden_nbt::binary::{FromModifiedUtf8, ToModifiedUtf8};
...
impl Region {
        ...
        S: for<'a> FromModifiedUtf8<'a> + Hash + Ord,
    {
        ...
        let (data, _) = kanden_nbt::from_binary(&mut nbt_slice)?;
        ...
        S: ToModifiedUtf8 + Hash + Ord,
    {
        ...
        match options.compression {
            Compression::Gzip => kanden_nbt::to_binary(
                chunk,
                GzEncoder::new(&mut compress_cursor, flate2::Compression::default()),
                Some(""),
            )?,
            Compression::Zlib => kanden_nbt::to_binary(
                chunk,
                ZlibEncoder::new(&mut compress_cursor, flate2::Compression::default()),
                Some(""),
            )?,
            Compression::None => kanden_nbt::to_binary(chunk, &mut compress_cursor, Some(""))?,
        }
