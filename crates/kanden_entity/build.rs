...
impl Value {
    ...
    fn field_type(&self) -> TokenStream {
        match self {
            ...
            Value::TextComponent(_) => quote!(kanden_protocol::Text),
            Value::OptionalTextComponent(_) => quote!(Option<kanden_protocol::Text>),
            Value::ItemStack(_) => quote!(kanden_protocol::ItemStack),
            ...
            Value::ParticleList(_) => {
                quote!(Vec<kanden_protocol::packets::play::level_particles_s2c::Particle>)
            }
    ...
    fn default_expr(&self) -> TokenStream {
        match self {
            ...
            Value::BlockPos(BlockPos { x, y, z }) => {
                quote!(kanden_protocol::BlockPos { x: #x, y: #y, z: #z })
            }
            ...
            Value::Particle(p) => match p.to_pascal_case().as_str() {
                ...
                "EntityEffect" => {
                    quote!(kanden_protocol::packets::play::level_particles_s2c::Particle::EntityEffect { color: 0 })
                }
                other => {
                    let variant = ident(other);
                    quote!(kanden_protocol::packets::play::level_particles_s2c::Particle::#variant)
                }
            },
...
fn build_entities() -> anyhow::Result<TokenStream> {
    ...
    Ok(quote! {
        use kanden_generated::attributes::EntityAttribute;
